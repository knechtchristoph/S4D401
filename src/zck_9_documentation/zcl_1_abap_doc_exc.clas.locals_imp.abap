...
CLASS lcl_carrier DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    "! @raising zcx_2_ck_failed | Instantiation failed - evaluate the exception text for details.
    ...
    "! @parameter e_days_later | Number of days after requested date
...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  METHOD get_instance.
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_2_ck_failed
        EXPORTING
          textid     = zcx_2_ck_failed=>carrier_not_exist
          carrier_id = i_carrier_id.
    ENDIF.
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_2_ck_failed
        EXPORTING
          textid     = zcx_2_ck_failed=>carrier_no_read_auth
          carrier_id = i_carrier_id.
    ENDIF.
  ...
  METHOD constructor.
    ...
    flights = VALUE #( BASE flights
                       FOR pflight IN passenger_flights ( pflight ) ).
    flights = VALUE #( BASE flights
                       FOR cflight IN cargo_flights ( cflight ) ).
